...
class Conv2DFCSigmoid(Router):
    def __init__(self, in_shape, convolutions=1, fully_connected=1, kernels=40, kernel_size=5, pad=0, dilation=1,
                 stride=1):
        ...
        for convolution in range(convolutions):
            conv = nn.Conv2d(in_channels=in_shapes[convolution], out_channels=kernels, kernel_size=kernel_size,
                             padding=pad, dilation=dilation, stride=stride)
        ...
        for fully_connect in range(fully_connected):
            if fully_connect == fully_connected-1:
                ...
                self.model.add_module("FC %i" % fully_connect, nn.Linear(kernels, kernels))
